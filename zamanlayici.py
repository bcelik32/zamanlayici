...
import ctypes
dosya_adi = "kapat2.jpg"
dosya_konumu = os.path.abspath(dosya_adi)
image_path =(dosya_konumu)
eastersayac=int(0)
# x = datetime.now()
# print(x.strftime("%X"))
...
def change_wallpaper(image_path):
    # Wallpaper'ı değiştirmek için Windows API'sini çağırıyoruz
...
def belirled():
    ...
    saniyeformul=saniyedonsumus*1
        ...
        #if (eastersayac >= 5):
      #      surebelirtiniz.config(text="""     Sayaç Kırılmaya
   #  Başladı""")
     #       if (eastersayac >= 8):
    #            saatglabel.config(text='S⋛at ⁘°ir i≭li s⋠i i≭z',font=("Vertana",11))

    #print(eastersayac)
    with open("durum.txt", "r+") as dosya:
        durum = dosya.read().strip()
    if durum=="True":
        surebelirtiniz.place(x=255,y=107)
        surebelirtiniz.config(text=f"""Zaten Bekleyen Bir
Kapatma İşelmi Var""")
        if (eastersayac >= 32):
            surebelirtiniz.config(text="""Zamanlayıcı Bozuldu 
╰（‵□′）╯""")
    if  durum=="False":
        eastersayac=0
        with open("durum.txt", "w") as dosya:
            dosya.seek(0)
            dosya.write("True")
            dosya.truncate()
        surebelirtiniz.config(text=f"""Oturumunuz {saatdonustur}:{dakikadonustur}:{saniyedonustur}
Kadar Sonra Kapatılacak""")
        #if (saatdonustur=="18") and (dakikadonustur=="24") and (saniyedonustur=="27"):
...
Belirtiniz""")
        ...
        sonformul=saatsaniye+dakikasaniye
        os.system(f"shutdown /s /t {sonformul}")
        surebelirtiniz.config(text=f"""Oturumunuz {saat2cagir}:{dakika2cagir}' da
Kapanacaktır""") 
...
def durdur():
    ...
    Edildi""")
        ...
        surebelirtiniz.config(text=f"""Zaten Kapatma İşlemi 
Mevcut Değil""")
    if (eastersayac >= 32):
        surebelirtiniz.config(text="""Zamanlayıcı Bozuldu 
╰（‵□′）╯""")
...
createdby.config(text="Created By bcelik32",font=("Lucida Calligraphy",11))
createdby.place(x=265,y=153)
# ikin4=Label(pencere)
# ikin4.pack()
